--[[
    Creator(s): TheSeaweedMonster
    Developer(s): I2rys#0663
    Source: https://github.com/TheSeaweedMonster/Celery
    Statuses:
        - [2023-04-22]
...
local function beautify(code)
    ...
    for i, line in ipairs(lines) do
        local is_closing_brace = line:find("^%s*%}")

        if is_closing_brace then indent -= 2 end

        result = result .. (" "):rep(indent) .. line .. "\n"

        local is_opening_brace = line:find("%b{}")

        if is_opening_brace then indent += 2 end
    end
...
getgenv().decompile = function(a) return disassemble(getscriptbytecode(a)) end